    ...
    <?php 
      ...
      ?>
        ...
        <div id="content_container1">
          ...
          <div id = 'header_lower'><form action = "search.php" method = "GET"><input type = 'text' placeholder="Поиск"  name = 'request'></div>
...
<?php
  ...
  while($result = mysqli_fetch_array($sql)){
    if($admin == 1){
      echo "<div  class = 'item'><span><object><embed src='fales/" . $result['id'] . ".pdf' width='400' height='300' /> </object></span><h2>{$result['name']}</h2><h3>Автор: {$result['author']} <a href = '?delete={$result['id']}'>Удалить</a></h3><a href = 'item.php?id={$result['id']}'>Больше</a></div>";
    } else {
